...
pub async fn list_directory(conn: Connection<'_, Db>, id: i32, key: Result<JWT, NetworkResponse>,)-> Result<Json<NetworkResponse>, Json<NetworkResponse>> {
    let key = match key {
    Ok(JWT { claims: c }) => Ok(c),
    _ => Err(Json(NetworkResponse::Unauthorized(
    "Requested unauthorized".to_string(),
    ...
    let response = match key{
        Ok(c) => {
            ...
            if(original_parent.is_none()){ //list root dir
                ...
                let childrens = dir_list.find_related(Files).all(db).await.unwrap();
                Ok(Json(NetworkResponse::Ok(json!(childrens).to_string())))
            }else{
                Err(Json(NetworkResponse::NotFound(String::from("Directory not found"))))
            }
        }
        Err(e) => Err(e)
    };
...
pub async fn get_directory_name_handler(conn: Connection<'_, Db>, id: i32,    key: Result<JWT, NetworkResponse>)-> Result<Json<NetworkResponse>, Json<NetworkResponse>>{
    let key = match key {
        Ok(JWT { claims: c }) => Ok(c),
        _ => Err(Json(NetworkResponse::Unauthorized(
            "Requested unauthorized".to_string(),
    ...
    let response = match key{
        Ok(c) => {
            ...
            if(check_dir_exists(&db, &id)).await{
                let name = Files::find_by_id(id).one(db).await.unwrap().unwrap().filename;
                Ok(Json(NetworkResponse::Ok(name)))
            }else{
                Err(Json(NetworkResponse::NotFound(String::from("Directory doesn't exists"))))
            }
        }
        Err(e) => Err(e)
    };
